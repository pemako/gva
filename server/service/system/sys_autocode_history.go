...
import (
	...
	systemReq "github.com/pemako/gva/server/model/system/request"
	"github.com/pemako/gva/server/utils/ast"
...
func (autoCodeHistoryService *AutoCodeHistoryService) CreateAutoCodeHistory(meta, structName, structCNName, autoCodePath string, injectionMeta string, tableName string, apiIds string, Package string) error {
	return global.GvaDb.Create(&system.SysAutoCodeHistory{
		Package:       Package,
		RequestMeta:   meta,
...
func (autoCodeHistoryService *AutoCodeHistoryService) First(info *request.GetById) (string, error) {
	var meta string
	return meta, global.GvaDb.Model(system.SysAutoCodeHistory{}).Select("request_meta").Where("id = ?", info.Uint()).First(&meta).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) Repeat(businessDB, structName, Package string) bool {
	var count int64
	global.GvaDb.Model(&system.SysAutoCodeHistory{}).Where("business_db = ? and struct_name = ? and package = ? and flag = 0", businessDB, structName, Package).Count(&count)
	return count > 0
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) RollBack(info *systemReq.RollBack) error {
	...
	if err := global.GvaDb.Where("id = ?", info.ID).First(&md).Error; err != nil {
		return err
	}
	// 清除API表
	...
	if info.DeleteTable {
		if err = AutoCodeServiceApp.DropTable(md.BusinessDB, md.TableName); err != nil {
			global.GvaLog.Error("ClearTag DropTable:", zap.Error(err))
		}
	}
	// 删除文件
	...
	for _, path := range strings.Split(md.AutoCodePath, ";") {
		...
		// 迁移
		nPath := filepath.Join(global.GvaConfig.AutoCode.Root,
		...
		if err != nil {
			global.GvaLog.Error("file move err ", zap.Error(err))
		}
	...
	md.Flag = 1
	return global.GvaDb.Save(&md).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) Delete(info *request.GetById) error {
	return global.GvaDb.Where("id = ?", info.Uint()).Delete(&system.SysAutoCodeHistory{}).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) GetList(info request.PageInfo) (list []response.AutoCodeHistory, total int64, err error) {
	...
	db := global.GvaDb.Model(&system.SysAutoCodeHistory{})
