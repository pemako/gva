...
import (
	...
	"github.com/pemako/gva/server/resource/autocode_template/subcontract"
	"github.com/pemako/gva/server/utils"
	ast2 "github.com/pemako/gva/server/utils/ast"
)
...
func Init(Package string) {
	injectionPaths = []injectionMeta{
		{
			path: filepath.Join(global.GvaConfig.AutoCode.Root,
				global.GvaConfig.AutoCode.Server, fmt.Sprintf(global.GvaConfig.AutoCode.SApi, Package), "enter.go"),
			funcName:    "ApiGroup",
			structNameF: "%sApi",
		},
		{
			path: filepath.Join(global.GvaConfig.AutoCode.Root,
				global.GvaConfig.AutoCode.Server, fmt.Sprintf(global.GvaConfig.AutoCode.SRouter, Package), "enter.go"),
			funcName:    "RouterGroup",
			structNameF: "%sRouter",
		},
		{
			path: filepath.Join(global.GvaConfig.AutoCode.Root,
				global.GvaConfig.AutoCode.Server, fmt.Sprintf(global.GvaConfig.AutoCode.SService, Package), "enter.go"),
	...
	packageInjectionMap = map[string]astInjectionMeta{
		packageServiceName: {
			path: filepath.Join(global.GvaConfig.AutoCode.Root,
				global.GvaConfig.AutoCode.Server, "service", "enter.go"),
			importCodeF:  "github.com/pemako/gva/server/%s/%s",
			packageNameF: "%s",
		...
		packageRouterName: {
			path: filepath.Join(global.GvaConfig.AutoCode.Root,
				global.GvaConfig.AutoCode.Server, "router", "enter.go"),
			importCodeF:  "github.com/pemako/gva/server/%s/%s",
			packageNameF: "%s",
		...
		packageAPIName: {
			path: filepath.Join(global.GvaConfig.AutoCode.Root,
				global.GvaConfig.AutoCode.Server, "api/v1", "enter.go"),
			importCodeF:  "github.com/pemako/gva/server/%s/%s",
			packageNameF: "%s",
			groupName:    "ApiGroup",
	...
	if strings.Contains(fileSlice[1], "server") {
		if strings.Contains(fileSlice[n-2], "router") {
			data.autoMoveFilePath = filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server,
				fmt.Sprintf(global.GvaConfig.AutoCode.SRouter, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "api") {
			data.autoMoveFilePath = filepath.Join(global.GvaConfig.AutoCode.Root,
				global.GvaConfig.AutoCode.Server, fmt.Sprintf(global.GvaConfig.AutoCode.SApi, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "service") {
			data.autoMoveFilePath = filepath.Join(global.GvaConfig.AutoCode.Root,
				global.GvaConfig.AutoCode.Server, fmt.Sprintf(global.GvaConfig.AutoCode.SService, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "model") {
			data.autoMoveFilePath = filepath.Join(global.GvaConfig.AutoCode.Root,
				global.GvaConfig.AutoCode.Server, fmt.Sprintf(global.GvaConfig.AutoCode.SModel, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "request") {
			data.autoMoveFilePath = filepath.Join(global.GvaConfig.AutoCode.Root,
				global.GvaConfig.AutoCode.Server, fmt.Sprintf(global.GvaConfig.AutoCode.SRequest, data.autoPackage), base)
		}
...
func (autoCodeService *AutoCodeService) CreatePackageTemp(packageName string) error {
	...
	for i, s := range pendingTemp {
		pendingTemp[i].path = filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, filepath.Clean(fmt.Sprintf(s.path, packageName)))
	}
	// 选择模板
...
func (autoCodeService *AutoCodeService) CreatePlug(plug system.AutoPlugReq) error {
	...
	for _, tpl := range tplFileList {
		...
		if strings.Index(pathArr[2], "tpl") < 0 {
			dirPath := filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, fmt.Sprintf(global.GvaConfig.AutoCode.SPlug, plug.Snake+"/"+pathArr[2]))
			os.MkdirAll(dirPath, 0755)
		}
		file := filepath.Join(global.GvaConfig.AutoCode.Root, global.GvaConfig.AutoCode.Server, fmt.Sprintf(global.GvaConfig.AutoCode.SPlug, plug.Snake+"/"+tpl[len(plugPath):len(tpl)-4]))
...
func (autoCodeService *AutoCodeService) InstallPlugin(file *multipart.FileHeader) (web, server int, err error) {
	...
	if webIndex != -1 {
		err = installation(paths[webIndex], global.GvaConfig.AutoCode.Server, global.GvaConfig.AutoCode.Web)
		if err != nil {
	...
	if serverIndex != -1 {
		err = installation(paths[serverIndex], global.GvaConfig.AutoCode.Server, global.GvaConfig.AutoCode.Server)
	}
	return webIndex, serverIndex, err
}
...
func installation(path string, formPath string, toPath string) error {
	...
	var form = filepath.ToSlash(global.GvaConfig.AutoCode.Root + formPath + "/" + path)
	var to = filepath.ToSlash(global.GvaConfig.AutoCode.Root + toPath + "/plugin/")
