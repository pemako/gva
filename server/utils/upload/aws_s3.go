...
func (*AwsS3) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	filename := global.GvaConfig.AwsS3.PathPrefix + "/" + fileKey
	...
	_, err := uploader.Upload(&s3manager.UploadInput{
		Bucket: aws.String(global.GvaConfig.AwsS3.Bucket),
		Key:    aws.String(filename),
		Body:   f,
	})
	if err != nil {
		global.GvaLog.Error("function uploader.Upload() Filed", zap.Any("err", err.Error()))
		return "", "", err
	}

	return global.GvaConfig.AwsS3.BaseURL + "/" + filename, fileKey, nil
}
...
func (*AwsS3) DeleteFile(key string) error {
	session := newSession()
	svc := s3.New(session)
	filename := global.GvaConfig.AwsS3.PathPrefix + "/" + key
	bucket := global.GvaConfig.AwsS3.Bucket
	...
	if err != nil {
		global.GvaLog.Error("function svc.DeleteObject() Filed", zap.Any("err", err.Error()))
...
func newSession() *session.Session {
	sess, _ := session.NewSession(&aws.Config{
		Region:           aws.String(global.GvaConfig.AwsS3.Region),
		Endpoint:         aws.String(global.GvaConfig.AwsS3.Endpoint), //minio在这里设置地址,可以兼容
		S3ForcePathStyle: aws.Bool(global.GvaConfig.AwsS3.S3ForcePathStyle),
		DisableSSL:       aws.Bool(global.GvaConfig.AwsS3.DisableSSL),
		Credentials: credentials.NewStaticCredentials(
			global.GvaConfig.AwsS3.SecretID,
			global.GvaConfig.AwsS3.SecretKey,
			"",
		),
	})
	return sess
